...
After running an analysis in the Streamlit app, use the **Generate Full Report**
button at the bottom of the results section. It produces a self-contained HTML
report (all parameters, critical speeds, bearing reactions, and every plot embedded
as images) that can be downloaded with one click and opened in any browser.

## Git Helper Scripts (Windows)

Batch files are included to simplify version control against
[`github.com/tbgallinucci/rotordynamics`](https://github.com/tbgallinucci/rotordynamics):

| Script | Action |
| --- | --- |
| `git_setup.bat` | One-time init: create the repo and link the `origin` remote |
| `git_commit.bat "message"` | Stage all changes and commit (prompts if no message given) |
| `git_push.bat` | Push commits to `origin/main` |
| `git_pull.bat` | Pull the latest changes from `origin/main` |
| `git_sync.bat "message"` | Commit + pull + push in one step |

Double-click a script, or run it from a terminal with a commit message argument.
